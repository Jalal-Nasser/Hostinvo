...
const copyByLocale: Record<AppLocale, Copy> = {
  en: {
    ...
    serverBody: "Connect your first cPanel or Plesk node before enabling production services.",
    productTitle: "Create your first product",
    productBody: "Define the first hosting offer and pricing model for your commercial launch.",
    ...
    items: [
      { key: "billing", title: "Automated billing", body: "Recurring invoices, gateway collection, credits, and reminders." },
      { key: "provisioning", title: "Server provisioning", body: "Queue-backed infrastructure workflows for cPanel and Plesk." },
      { key: "domains", title: "Domain management", body: "Lifecycle tracking for registrations, renewals, and contacts." },
      { key: "support", title: "Client support", body: "Integrated tickets, internal notes, and provider operations." },
    ],
  },
  ar: {
    ...
    items: [
      { key: "billing", title: "فوترة تلقائية", body: "فواتير متكررة وتحصيل عبر البوابات وأرصدة وتذكيرات." },
      { key: "provisioning", title: "تزويد الخوادم", body: "مسارات عمل مدعومة بالطوابير لخوادم cPanel وPlesk." },
      { key: "domains", title: "إدارة النطاقات", body: "تتبع كامل للتسجيلات والتجديدات وجهات الاتصال." },
      { key: "support", title: "دعم العملاء", body: "نظام تذاكر مدمج وملاحظات داخلية وتشغيل دعم احترافي." },
    ],
  },
};
...
function CompatibilityLogo({ platform }: { platform: "cpanel" | "plesk" | "linux" }) {
  const icon =
...
export function ProviderOnboardingWizard({ locale }: ProviderOnboardingWizardProps) {
  ...
  const [registerForm, setRegisterForm] = useState({ name: "", email: "", password: "", passwordConfirmation: "", companyName: "", companyDomain: "", licenseKey: "", licenseInstance: "" });
  const [companyForm, setCompanyForm] = useState({ companyName: "", companyDomain: "", defaultCurrency: "USD", timezone: "UTC" });

  const completed = status ? status.steps.filter((step) => step.complete).length : 0;
  ...
  function submitCreate(event: React.FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    setMessage(null);
    const host = typeof window !== "undefined" ? window.location.hostname : registerForm.companyDomain;
    startTransition(async () => {
      const response = await registerProvider({
        ...
        timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || companyForm.timezone || "UTC",
        license_key: registerForm.licenseKey || undefined,
        license_domain: registerForm.licenseKey && (registerForm.companyDomain || host) ? registerForm.companyDomain || host : undefined,
        license_instance_id: registerForm.licenseKey && registerForm.licenseInstance ? registerForm.licenseInstance : registerForm.licenseKey ? "provider-instance-1" : undefined,
      });
      if (!response.ok) {
  ...
  const stepTitle = currentStep === 1 ? copy.createTitle : currentStep === 2 ? copy.configureTitle : currentStep === 3 ? copy.serverTitle : copy.productTitle;
  const stepBody = currentStep === 1 ? copy.createBody : currentStep === 2 ? copy.configureBody : currentStep === 3 ? copy.serverBody : copy.productBody;
    ...
    <div dir={isRtl ? "rtl" : "ltr"} className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(4,141,254,0.12),transparent_24rem),linear-gradient(180deg,#f8fbff_0%,#eef4fb_100%)] text-[#0a1628]">
      <header className="border-b border-[#dbe8f7] bg-white/90 backdrop-blur">
        <div className="mx-auto flex w-full max-w-[88rem] items-center justify-between gap-6 px-6 py-5 sm:px-8 xl:px-10">
          ...
          <div className="flex items-center gap-4">
            ...
            {!isAuthenticated ? <Link href={localePath(locale, "/auth/login")} className="text-sm font-semibold text-[#33506f] transition hover:text-[#036deb]">{copy.login}</Link> : null}
          </div>
        </div>
      </header>
              ...
              return (
                <div key={label} className={`${active ? "border-[#a6d5ff] bg-white shadow-[0_18px_40px_rgba(4,109,235,0.08)]" : "border-[#deebf8] bg-white/70"} flex items-center gap-4 rounded-[1.5rem] border px-4 py-4 transition`}>
                  ...
                  <div>
                    ...
                    <div className="mt-1 text-sm font-semibold text-[#0a1628]">{label}</div>
                  </div>
                </div>
              );
            })}
      ...
      <main className="mx-auto grid w-full max-w-[88rem] gap-7 px-6 py-8 sm:px-8 xl:grid-cols-[minmax(0,1.14fr)_21rem] xl:items-start xl:px-10">
        ...
        <section className={`${card} self-start p-6 sm:p-8`}>
          {message ? <div className="mb-5 rounded-[1rem] border border-[#ccebd8] bg-[#f2fbf6] px-4 py-3 text-sm leading-6 text-[#1f7a46]">{message}</div> : null}
          {error ? <div className="mb-5 rounded-[1rem] border border-[#ffd5d2] bg-[#fff4f3] px-4 py-3 text-sm leading-6 text-[#b7382d]">{error}</div> : null}
          <div className="border-b border-[#e3edf8] pb-6">
            <div className="inline-flex items-center gap-3 rounded-full border border-[#cfe0f4] bg-[#f8fbff] px-3 py-2 text-sm font-medium text-[#4e6782]">
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[linear-gradient(135deg,#048DFE,#036DEB)] text-sm font-semibold text-white">{currentStep}</div>
              <span>{stepTitle}</span>
            </div>
            <p className="mt-4 max-w-2xl text-sm leading-7 text-[#57718e]">{stepBody}</p>
          </div>

          {currentStep === 1 ? (
            <div className="mt-6 grid gap-5">
              <div className="text-sm leading-7 text-[#5a738f]">{copy.hint} <Link href={localePath(locale, "/auth/login")} className="font-semibold text-[#036deb] transition hover:text-[#002d8e]">{copy.login}</Link></div>
              <form onSubmit={submitCreate} className="grid gap-5">
                <div className="grid gap-5 md:grid-cols-2">
                  <div className="grid gap-2"><label className="text-sm font-semibold text-[#123055]">{copy.name}</label><input className={input} placeholder={copy.namePlaceholder} value={registerForm.name} onChange={(e) => setRegisterForm((v) => ({ ...v, name: e.target.value }))} required /></div>
                  <div className="grid gap-2"><label className="text-sm font-semibold text-[#123055]">{copy.email}</label><input className={input} type="email" placeholder={copy.emailPlaceholder} value={registerForm.email} onChange={(e) => setRegisterForm((v) => ({ ...v, email: e.target.value }))} required /></div>
                </div>
                <div className="grid gap-5 md:grid-cols-2">
                  <div className="grid gap-2"><label className="text-sm font-semibold text-[#123055]">{copy.password}</label><input className={input} type="password" placeholder={copy.passwordPlaceholder} value={registerForm.password} onChange={(e) => setRegisterForm((v) => ({ ...v, password: e.target.value }))} required /></div>
                  <div className="grid gap-2"><label className="text-sm font-semibold text-[#123055]">{copy.confirmPassword}</label><input className={input} type="password" placeholder={copy.confirmPasswordPlaceholder} value={registerForm.passwordConfirmation} onChange={(e) => setRegisterForm((v) => ({ ...v, passwordConfirmation: e.target.value }))} required /></div>
                </div>
                <div className="grid gap-5 md:grid-cols-2">
                  <div className="grid gap-2"><label className="text-sm font-semibold text-[#123055]">{copy.companyName}</label><input className={input} placeholder={copy.companyNamePlaceholder} value={registerForm.companyName} onChange={(e) => setRegisterForm((v) => ({ ...v, companyName: e.target.value }))} required /></div>
                  <div className="grid gap-2"><label className="text-sm font-semibold text-[#123055]">{copy.companyDomain}</label><input className={input} placeholder={copy.companyDomainPlaceholder} value={registerForm.companyDomain} onChange={(e) => setRegisterForm((v) => ({ ...v, companyDomain: e.target.value }))} /></div>
                </div>
                <div className="rounded-[1.35rem] border border-[#dbe8f7] bg-[#f8fbff] p-4">
                  ...
                  {showLicense ? (
                    <div className="mt-4 grid gap-4 md:grid-cols-2">
                      ...
                      <div className="grid gap-2"><label className="text-sm font-semibold text-[#123055]">{copy.licenseInstance}</label><input className={input} placeholder={copy.licenseInstancePlaceholder} value={registerForm.licenseInstance} onChange={(e) => setRegisterForm((v) => ({ ...v, licenseInstance: e.target.value }))} /></div>
                    </div>
                  ) : null}
                </div>
                <button type="submit" disabled={isPending} className={`${primary} w-full`}>{isPending ? "..." : copy.createSubmit}</button>
              </form>
              {!isAuthenticated ? <div className="rounded-[1rem] border border-[#d8e7f6] bg-[#f8fbff] px-4 py-3 text-sm leading-6 text-[#57718e]">{copy.unauthenticated}</div> : null}
            </div>
          ) : null}
          ...
          {currentStep === 2 ? (
            <form onSubmit={submitCompany} className="mt-6 grid gap-5">
              <div className="grid gap-5 md:grid-cols-2">
                <div className="grid gap-2"><label className="text-sm font-semibold text-[#123055]">{copy.companyName}</label><input className={input} placeholder={copy.companyNamePlaceholder} value={companyForm.companyName} onChange={(e) => setCompanyForm((v) => ({ ...v, companyName: e.target.value }))} required /></div>
                <div className="grid gap-2"><label className="text-sm font-semibold text-[#123055]">{copy.companyDomain}</label><input className={input} placeholder={copy.companyDomainPlaceholder} value={companyForm.companyDomain} onChange={(e) => setCompanyForm((v) => ({ ...v, companyDomain: e.target.value }))} required /></div>
              </div>
              <div className="grid gap-5 md:grid-cols-2">
                <div className="grid gap-2"><label className="text-sm font-semibold text-[#123055]">{copy.currency}</label><input className={input} placeholder={copy.currencyPlaceholder} value={companyForm.defaultCurrency} onChange={(e) => setCompanyForm((v) => ({ ...v, defaultCurrency: e.target.value.toUpperCase() }))} required /></div>
                <div className="grid gap-2"><label className="text-sm font-semibold text-[#123055]">{copy.timezone}</label><input className={input} placeholder={copy.timezonePlaceholder} value={companyForm.timezone} onChange={(e) => setCompanyForm((v) => ({ ...v, timezone: e.target.value }))} required /></div>
              </div>
              <button type="submit" disabled={isPending || !isAuthenticated} className={`${primary} w-full`}>{isPending ? "..." : copy.configureSubmit}</button>
            </form>
          ) : null}

          {currentStep === 3 ? <div className="mt-6 grid gap-5"><div className="rounded-[1.35rem] border border-[#dbe8f7] bg-[#f8fbff] p-5 text-sm leading-7 text-[#57718e]">{copy.serverBody}</div><div className="flex flex-wrap gap-3"><Link href={localePath(locale, "/dashboard/servers")} className={primary}>{copy.openServers}</Link><button type="button" onClick={refreshStatus} className={secondary}>{copy.refresh}</button></div></div> : null}
          {currentStep === 4 ? <div className="mt-6 grid gap-5"><div className="rounded-[1.35rem] border border-[#dbe8f7] bg-[#f8fbff] p-5 text-sm leading-7 text-[#57718e]">{copy.productBody}</div><div className="flex flex-wrap gap-3"><Link href={localePath(locale, "/dashboard/products/new")} className={primary}>{copy.openProducts}</Link><button type="button" onClick={refreshStatus} className={secondary}>{copy.refresh}</button></div></div> : null}
        </section>
        ...
        <aside className="grid gap-5 xl:sticky xl:top-8 xl:self-start">
          <div className={`${card} p-6`}>
            <div className="text-[11px] font-semibold uppercase tracking-[0.26em] text-[#5f7ca5]">{copy.progress}</div>
            ...
            <div className="mt-6">
              <div className="mb-2 flex items-center justify-between text-xs font-medium text-[#7390af]"><span>{completed} {copy.complete}</span><span>{Math.round((completed / labels.length) * 100)}%</span></div>
              <div className="h-2 overflow-hidden rounded-full bg-[#e4eef8]"><div className="h-2 rounded-full bg-[linear-gradient(135deg,#048DFE,#036DEB)] transition-all duration-500" style={{ width: `${(completed / labels.length) * 100}%` }} /></div>
            </div>
          </div>

          <div className={`${card} p-6`}>
            <div className="text-[11px] font-semibold uppercase tracking-[0.26em] text-[#5f7ca5]">{copy.included}</div>
            <div className="mt-5 grid gap-4">
              {copy.items.map((item) => (
                <div key={item.key} className="rounded-[1.25rem] border border-[#e4eef8] bg-[#f9fcff] p-4">
                  <div className="text-sm font-semibold text-[#0a1628]">{item.title}</div>
                  <div className="mt-1.5 text-sm leading-6 text-[#58718c]">{item.body}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mx-auto flex w-full max-w-[88rem] flex-col gap-8 px-6 py-8 sm:px-8 xl:px-10">
          <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-center">
            <div className="max-w-2xl">
              <BrandLogo href={`/${locale}`} className="block w-full max-w-[11rem]" />
